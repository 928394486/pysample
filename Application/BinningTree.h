...
public:
	...
	BinningTreeNode(Box&& b, std::vector<std::weak_ptr<MinGrid>>&& vec, StatisticalInfo&& info, 
		std::weak_ptr<BinningTreeNode> parent) : b(std::move(b)), min_grids_inside(std::move(vec)), 
		info(std::move(info)), parent(parent)
	{
		child1 = nullptr;
		child2 = nullptr;
		seed_index = -1;
